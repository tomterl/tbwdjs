...
module.exports = {
    ...
    testUpdateInfoUser: function(test) {
    	...
    	var userInfo = this.api.updateUserInfo(data, function(r) {
	    	var userInfo = that.api.getUserInfo(function(response) {
		    	test.equal(firstName, response.first_name);
		    	test.done();
		    });
	    });
    },
    testListTests: function(test) {
    	var list = this.api.getTests(function(response) {
	    	test.ok(response.data.length > 0, true);
	    	test.done();
	    });
    },
    testInfoSpecificTest: function(test) {
    	...
    	var testInfo = this.api.getTests(function(response) {
	    	...
	    	that.api.getTestDetails(singleTest["id"], function(response) {
	    		test.equal(response["session_id"], singleTest["session_id"]);
	    		test.done();
	    	});
	    });
    },
    ...
    testUpdateTest: function(test) {
    	...
    	var testInfo = this.api.getTests(function(response) {
	    	...
	    	that.api.updateTest(newTestData, singleTest["id"], function(response) {
	    		that.api.getTestDetails(singleTest["id"], function(response) {
		    		test.equal(response["name"], newTestName);
		    		test.done();
		    	});
	    	});
	    });
    ...
    testUpdateTestNotFound: function(test) {
    	...
    	var update = this.api.updateTest(newTestData, 324324234, function(response) {
    		test.equal(null, response);
    		test.done();
    	});
    }
